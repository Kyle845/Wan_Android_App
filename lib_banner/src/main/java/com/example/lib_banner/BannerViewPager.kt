...
import com.example.lib_banner.annotation.*
import com.example.lib_banner.base.BaseBannerAdapter
import com.example.lib_banner.base.BaseViewHolder
import com.example.lib_banner.indicator.IIndicator
import com.example.lib_banner.indicator.IndicatorView
import com.example.lib_banner.manager.BannerManager
import com.example.lib_banner.manager.ReflectLayoutManager
import com.example.lib_banner.mode.PageStyle
import com.example.lib_banner.options.BannerOptions
...
import com.example.lib_banner.utils.BannerUtils
import com.example.lib_banner.utils.BannerUtils.getOriginalPosition
...
open class BannerViewPager<T, H : BaseViewHolder<T>> @JvmOverloads constructor(
    ...
    private fun setupViewPager(list: List<T>, isInitCurrent: Boolean = true) {
        ...
        mViewPager?.offscreenPageLimit = if (limit > 0) limit else ViewPager2.OFFSCREEN_PAGE_LIMIT_DEFAULT
    ...
    fun refreshData(list: List<T>?) {
        post {
            if (isAttachedToWindow && (list != null) && (mBannerPagerAdapter != null)) {
                ...
                mViewPager?.offscreenPageLimit = if (limitSize > 0) limitSize else ViewPager2.OFFSCREEN_PAGE_LIMIT_DEFAULT
